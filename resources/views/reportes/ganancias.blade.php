...
@section('content')
    ...
    <div class="cards">
        <div class="stat blue">
            ...
            <div><div class="num">{{ $moneda }} {{ number_format($ingresos, 2) }}</div><div class="label">Ingresos (sin IVA)</div></div>
            <i class="fa-solid fa-arrow-trend-up bg-ico"></i>
        </div>
        <div class="stat orange">
            <i class="fa-solid fa-money-bill-wave icon"></i>
            <div><div class="num">{{ $moneda }} {{ number_format($costo, 2) }}</div><div class="label">Costo de ventas</div></div>
            <i class="fa-solid fa-money-bill-wave bg-ico"></i>
        </div>
        <div class="stat purple">
            <i class="fa-solid fa-coins icon"></i>
            <div><div class="num">{{ $moneda }} {{ number_format($utilidad, 2) }}</div><div class="label">Utilidad bruta</div></div>
        ...
        <table class="table">
            ...
            @forelse($detalle as $d)
                ...
                <tr>
                    ...
                    <td>{{ $moneda }} {{ number_format($d->ingresos, 2) }}</td>
                    <td>{{ $moneda }} {{ number_format($d->costo, 2) }}</td>
                    <td><strong style="color:{{ $d->utilidad >= 0 ? '#1e9e5a' : '#e74c3c' }}">{{ $moneda }} {{ number_format($d->utilidad, 2) }}</strong></td>
