...
@section('content')
    ...
    <div class="cards">
        <div class="stat blue">
            ...
            <div>
                <div class="num">{{ $moneda }} {{ number_format($ventasHoy, 2) }}</div>
        ...
        <div class="stat orange">
            ...
            <div>
                <div class="num">{{ $moneda }} {{ number_format($ventasMes, 2) }}</div>
    ...
    @if(($lotesPorVencer > 0 || $lotesVencidos > 0))
        <div class="panel" style="border-left:4px solid #e67e22;margin-bottom:18px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
            <span style="color:#d35400;font-weight:600">
                <i class="fa-solid fa-calendar-xmark" style="font-size:1.2rem;margin-right:6px"></i>
                @if($lotesVencidos > 0)
                    <strong>{{ $lotesVencidos }} lote(s) VENCIDO(S)</strong> y 
                @endif
                {{ $lotesPorVencer }} lote(s) próximo(s) a vencer este mes.
            </span>
            <a href="{{ route('lotes.index') }}" class="btn btn-primary btn-sm" style="width:auto;text-decoration:none">
                <i class="fa-solid fa-boxes-stacked"></i> Ver Lotes FEFO
            </a>
        </div>
    @endif
