...
@php
    ...
    $tasaIgvStr = rtrim(rtrim(number_format($empresa->igv ?? 12, 2), '0'), '.');
@endphp
...
@section('content')
    ...
    <div class="toolbar no-print">
        ...
        @if($venta->fe_estado && $venta->fe_estado !== 'NO_APLICA')
            ...
            <a href="{{ route('facturacion.ticket', $venta) }}" target="_blank" class="btn btn-light btn-sm" style="width:auto">
                <i class="fa-solid fa-receipt"></i> Ticket FE
            </a>
    ...
    <script>
        function anularVenta(){
            @if($venta->feEmitido())
            var motivo = prompt('Motivo de la anulación:', 'ANULACION DE LA OPERACION');
    ...
    <div class="ticket">
        ...
        @if($empresa && ($empresa->ruc || $empresa->direccion))
            <div class="sub" style="margin-bottom:4px">
                @if($empresa->ruc)NIT / RUC: {{ $empresa->ruc }}@endif
        ...
        <div class="meta">
            ...
            <div><span>Cliente:</span><span>{{ $venta->cliente->nombre ?? 'Consumidor final' }}</span></div>
            ...
            @if($venta->fe_estado && $venta->fe_estado !== 'NO_APLICA')
                @if($venta->comprobanteElectronico())
                    <div><span>Comprobante DTE:</span><span>{{ $venta->comprobanteElectronico() }}</span></div>
                @endif
                <div><span>Estado DTE:</span><span>{{ $venta->feEstadoLabel() }}</span></div>
            @endif
        </div>
        ...
        <div class="tot">
            <div class="r"><span>Subtotal (con IVA)</span><span>{{ $moneda }} {{ number_format($venta->subtotal,2) }}</span></div>
            @if($venta->descuento > 0)
                <div class="r"><span>Descuento</span><span>- {{ $moneda }} {{ number_format($venta->descuento,2) }}</span></div>
            @endif
            @if($venta->impuesto > 0)
                <div class="r"><span>Monto Gravable (sin IVA)</span><span>{{ $moneda }} {{ number_format($venta->total - $venta->impuesto,2) }}</span></div>
                <div class="r"><span>IVA incluido ({{ $tasaIgvStr }}%)</span><span>{{ $moneda }} {{ number_format($venta->impuesto,2) }}</span></div>
            @endif
            <div class="r big"><span>TOTAL</span><span>{{ $moneda }} {{ number_format($venta->total,2) }}</span></div>
            @if($venta->metodo_pago === 'EFECTIVO' && $venta->efectivo_recibido !== null)
                <div class="r"><span>Efectivo recibido</span><span>{{ $moneda }} {{ number_format($venta->efectivo_recibido,2) }}</span></div>
                <div class="r"><span>Vuelto</span><span>{{ $moneda }} {{ number_format($venta->vuelto,2) }}</span></div>
            @endif
        </div>
