...
@php
    $moneda = $compra->empresa->moneda ?? 'Q';
    $tasaIgvStr = rtrim(rtrim(number_format($compra->empresa->igv ?? 12, 2), '0'), '.');
@endphp
    ...
    <div class="grid-2">
        <div class="panel">
            ...
            <table class="table">
                ...
                <tr><td><strong>NIT / RUC</strong></td><td>{{ $compra->proveedor->ruc ?? '—' }}</td></tr>
        ...
        <div class="panel">
            ...
            <div class="totales">
                <div class="r"><span>Subtotal</span><span>{{ $moneda }} {{ number_format($compra->subtotal, 2) }}</span></div>
                <div class="r"><span>IVA ({{ $tasaIgvStr }}%)</span><span>{{ $moneda }} {{ number_format($compra->impuesto, 2) }}</span></div>
                <div class="r total"><span>Total</span><span>{{ $moneda }} {{ number_format($compra->total, 2) }}</span></div>
            </div>
        </div>
    </div>
