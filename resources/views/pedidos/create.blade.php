...
@section('title', 'Toma de Pedido en Ruta')
...
@section('content')
    <div class="page-head">
        <h1><i class="fa-solid fa-truck-fast" style="color:#e67e22"></i> TOMA DE PEDIDO EN RUTA</h1>
        <div class="breadcrumb">
            <a href="{{ route('dashboard') }}">Inicio</a>
            <span class="sep">/</span> <a href="{{ route('pedidos.index') }}">Pedidos</a>
            <span class="sep">/</span> Nuevo Pedido en Ruta
        </div>
    </div>
    ...
    <div class="form-card" style="max-width:100%">
        <form id="formPedidoRuta">
            @csrf
            <div class="form-grid">
                <div class="form-group">
                    <label>Cliente <span style="color:#e74c3c">*</span></label>
                    <select id="cliente_id" class="form-control" onchange="onClienteSelected()" required>
                        <option value="">— Seleccionar Cliente —</option>
                        @foreach($clientes as $c)
                            <option value="{{ $c->id }}"
                                data-rutaid="{{ $c->ruta_id }}"
                                data-rutanombre="{{ $c->ruta?->nombre }}"
                                data-rutadias="{{ $c->ruta?->dias_entrega_estimados }}"
                                data-doc="{{ $c->numero_documento }}">
                                {{ $c->nombre }} {{ $c->numero_documento ? '('.($c->tipo_documento ?? 'NIT/DPI').': '.$c->numero_documento.')' : '' }} {{ $c->ruta ? '📍 ('.$c->ruta->nombre.')' : '' }}
                            </option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label>Ruta de Entrega <span class="badge" style="background:#e0f2fe; color:#0369a1; font-size:10px; margin-left:6px;"><i class="fa-solid fa-lock"></i> Bloqueado (Desde Ficha de Cliente)</span></label>
                    <input type="text" id="txt_ruta_nombre" class="form-control" value="— Seleccionar Cliente —" readonly style="background:#f8fafc; color:#1e293b; font-weight:600;">
                    <input type="hidden" id="ruta_id" value="">
                </div>
                <div class="form-group">
                    <label>Fecha de Toma (Automática)</label>
                    <input type="text" class="form-control" value="{{ now()->format('d/m/Y H:i') }}" readonly style="background:#f8fafc; color:#64748b; font-weight:600;">
                </div>
                <div class="form-group">
                    <label>Fecha Estimada de Entrega</label>
                    <input type="date" id="fecha_entrega_estimada" class="form-control" value="{{ now()->addDay()->format('Y-m-d') }}">
                </div>
                <div class="form-group full">
                    <label>Observación / Nota del Pedido</label>
                    <input type="text" id="observacion" class="form-control" placeholder="Ej: Entregar en horario matutino, pago contra entrega, etc.">
                </div>
            </div>

            <h3 style="margin:22px 0 12px;color:#4a5560;font-size:14px"><span class="dot" style="display:inline-block;width:8px;height:8px;border-radius:50%;background:var(--brand);margin-right:6px"></span> Productos del Pedido</h3>
            ...
            <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:12px">
                    ...
                    <select id="prodSelect" class="form-control" onchange="onProductoSelected()">
                        ...
                        @foreach($productos as $prod)
                            <option value="{{ $prod->id }}"
                                ...
                                {{ $prod->codigo }} · {{ $prod->nombre }} {{ $prod->tiene_variantes ? '🎨 [Multi-sabor]' : '' }} {{ $prod->tiene_empaque ? '📦 ['.$prod->nombre_empaque.' x'.$prod->cant_por_empaque.']' : '' }} {{ $prod->es_perecedero ? '🥦 (Perecedero)' : '' }} - Disponibles: {{ $prod->stock }} {{ $prod->unidad }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div id="wrapLoteSelect" style="flex:1.5;min-width:200px;display:none;">
                    <select id="loteSelect" class="form-control" style="border-color:#e67e22;">
                        <option value="">Auto FEFO (Lote más próximo a vencer)</option>
                    </select>
                </div>
                ...
                <table class="table" id="tablaItems">
                    <thead>
                        <tr>
                            <th>Código</th>
                            <th>Producto</th>
                            <th style="width:200px">Lote Reservado</th>
                    ...
                    <tbody id="items">
                        <tr id="sinItems"><td colspan="7" style="text-align:center;color:#9aa3ab;padding:20px">Agrega los productos pedidos por el cliente.</td></tr>
                    </tbody>
                </table>
            </div>

            <div style="display:flex;justify-content:space-between;align-items:center;margin-top:20px;flex-wrap:wrap;gap:15px">
                <div style="background:#fef3c7;border:1px solid #f59e0b;padding:10px 15px;border-radius:8px;color:#92400e;font-size:0.85rem;">
                    <i class="fa-solid fa-lock"></i> <strong>Reserva de Stock:</strong> Al registrar este pedido, los productos y lotes seleccionados se reservarán de inmediato para evitar desabastecimiento.
                </div>
                ...
                <div class="totales" style="min-width:260px">
                    ...
                    <div class="r total"><span>Total Estimado</span><span id="t_total">{{ $simMoneda }} 0.00</span></div>
                </div>
            </div>

            <div class="form-actions" style="margin-top:20px">
                <button type="button" class="btn btn-success" style="width:auto" id="btnGuardar" onclick="guardarPedido()" disabled>
                    <i class="fa-solid fa-floppy-disk"></i> Registrar y Reservar Stock
                </button>
    ...
    <div id="modalVariantesBloque" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:9999; align-items:center; justify-content:center; padding:15px;">
        <div style="background:#fff; border-radius:12px; width:100%; max-width:650px; max-height:85vh; overflow-y:auto; padding:20px; box-shadow:0 20px 25px -5px rgba(0,0,0,0.2);">
            ...
            <div style="display:flex; justify-content:flex-end; gap:10px; margin-top:20px; border-top:1px solid #e2e8f0; padding-top:12px;">
                ...
                <button type="button" class="btn btn-warning" onclick="confirmarVariantesBloque()"><i class="fa-solid fa-plus"></i> Agregar Sabores al Pedido</button>
            </div>
        </div>
    </div>
@endsection
...
const URL_STORE = "{{ route('pedidos.store') }}";
...
function abrirModalVariantesBloque() {
    ...
    let html = `
        <div style="margin-bottom:12px; font-size:12px; color:#64748b;">
            Ingresa las cantidades solicitadas para cada sabor. Se agregarán sólo los sabores con cantidad mayor a 0.
        ...
        <table class="table" style="width:100%; font-size:13px; border-collapse:collapse;">
            <thead>
                <tr style="background:#f8fafc; text-align:left;">
                    ...
                    <th style="padding:8px; width:120px;">Cant. Pedido</th>
...
function confirmarVariantesBloque() {
    ...
    rows.forEach(r => {
        ...
        if (cant > 0) {
            agregarLineaPedidoIndividual(opt, varId, varSabor, varStk, cant, precio);
            agregados++;
        }
    });
...
function addLinea() {
    ...
    agregarLineaPedidoIndividual(opt, saborId, saborNombre, stockBase, 1, precioPres);
...
function agregarLineaPedidoIndividual(opt, saborId, saborNombre, stockBase, cantVal, precioPres) {
    const presSel = document.getElementById('presentacionSelect');
    const esEmpaque = (opt.dataset.tieneempaque === '1') && (presSel.value === 'EMPAQUE');
    const factor = esEmpaque ? (parseInt(opt.dataset.cantempaque) || 1) : 1;
    ...
    const loteTxt = loteSel.selectedIndex > 0 ? loteSel.options[loteSel.selectedIndex].text : 'Auto FEFO (Próximo a vencer)';
    ...
    tr.innerHTML = `
        ...
        <td>
            ...
            <br><span style="color:#9aa3ab;font-size:11px">Stock disp: ${stockBase} ${opt.dataset.unidad}</span>
        </td>
...
function onClienteSelected() {
    ...
    const opt = sel.options[sel.selectedIndex];
    const txtRuta = document.getElementById('txt_ruta_nombre');
    const hiddenRuta = document.getElementById('ruta_id');

    if (!opt.value) {
        txtRuta.value = '— Seleccionar Cliente —';
        hiddenRuta.value = '';
        return;
    }

    const rutaId = opt.dataset.rutaid || '';
    const rutaNombre = opt.dataset.rutanombre || '';
    const rutaDias = parseInt(opt.dataset.rutadias) || 1;

    if (rutaId) {
        txtRuta.value = `${rutaNombre} (Entrega est.: ${rutaDias} ${rutaDias === 1 ? 'día' : 'días'})`;
        hiddenRuta.value = rutaId;

        // Auto-calcular fecha estimada de entrega
        const fecha = new Date();
        fecha.setDate(fecha.getDate() + rutaDias);
        const yyyy = fecha.getFullYear();
        const mm = String(fecha.getMonth() + 1).padStart(2, '0');
        const dd = String(fecha.getDate()).padStart(2, '0');
        document.getElementById('fecha_entrega_estimada').value = `${yyyy}-${mm}-${dd}`;
    } else {
        txtRuta.value = '— Sin Ruta Asignada (Asignar en Ficha de Cliente) —';
        hiddenRuta.value = '';
    }
}
...
async function guardarPedido() {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            msg.textContent = data.message || 'Error al registrar el pedido.';
            btn.disabled = false;
            return;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    initSearchableSelect('cliente_id', { placeholder: 'Buscar cliente por nombre, NIT/DPI o ruta...' });
