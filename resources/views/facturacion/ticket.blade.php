...
<body>
@php use App\Support\NumeroALetras; $moneda = $venta->empresa->moneda ?? 'Q'; @endphp
...
<div class="ticket">
    ...
    <div class="c muted">NIT / RUC: {{ $cfg->ruc ?: '—' }}</div>
    @if($cfg->direccion_fiscal)<div class="c muted">{{ $cfg->direccion_fiscal }}</div>@endif

    <div class="doc">
        {{ $venta->comprobanteTitulo() }}<br>
        {{ $venta->fel_uuid ? 'FEL UUID: '.$venta->fel_uuid : ($venta->comprobanteElectronico() ?? $venta->numero) }}
    </div>
    ...
    @if($venta->cliente && $venta->cliente->numero_documento)
        <div>NIT / Doc: {{ $venta->cliente->numero_documento }}</div>
    @endif
    ...
    <div class="tot"><span>IVA</span><span>{{ $moneda }} {{ number_format($venta->impuesto,2) }}</span></div>
    ...
    <div class="c muted">Estado SAT / Emisión: {{ $venta->fe_estado ?? 'ACEPTADO' }}</div>
